  ...
  const CustomTooltip = ({ active, payload }) => {
    ...
    return (
      <div className="rounded-2xl border border-white/20 bg-white/90 px-3 py-2 text-xs text-slate-600 shadow-sm backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/90 dark:text-slate-300">
        ...
        {datum.credit > 0 && <p className="text-emerald-500">Inflows: ₹{Math.round(datum.credit).toLocaleString()} ({inflowPct}%)</p>}
        {datum.debit > 0 && <p className="text-rose-500">Outflows: ₹{Math.round(datum.debit).toLocaleString()} ({outflowPct}%)</p>}
        {datum.net !== 0 && <p className="text-slate-700 dark:text-slate-200">Net: ₹{Math.round(datum.net).toLocaleString()}</p>}
      </div>
    )
  }
...
export const DashboardAnalytics = () => {
  ...
  const basePieSegments = useMemo(() => {
    ...
    return limited.map((item, index) => {
      ...
      return {
        label: item.label,
        value: Math.max(1, Math.round(Math.max(Math.abs(debit), Math.abs(credit)))),
